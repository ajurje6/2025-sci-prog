...
1.  **Build a "Skill":** using Anthropic's protocol, create a modular skill for Data Cleaning and benchmark it against a vanilla prompt.
2.  **Prompt Optimization:** Replicate the Arize paper. Use a strong model (Judge) to iteratively rewrite the system prompt for a weak model (Student) on a specific dataset.
3.  **Hack the Eval:** Create a dataset of coding problems where a naive agent would be tempted to "Reward Hack," and build a grader that detects it.
